...
@section('content')
    ...
    <section class="header-page container border my-5 p-3">
        ...
        <span>/</span>
        <span class="main-font">{{ __('messages.register') }}</span>
    </section>
    ...
    <section class="sgin-form container border mb-5">
        <form action="{{ route('website.register.post') }}" method="post">
            ...
            <select class="main-font form-control rounded-0 my-4" name="governorate_id" id="governorate_id">
                <option value="" disabled {{ old('governorate_id') ? '' : 'selected' }}>
            ...
            <button type="submit"
                class="main-font btn border rounded-0 d-block mx-auto">{{ __('messages.create') }}</button>
        </form>
    </section>
