...
@section('content')
    <section class="sgin-form container border mb-5">
        <form action="{{ route('website.donations.store') }}" method="post">
            ...
            @error('patient_age')<div class="text-danger">{{ $message }}</div>@enderror
            ...
            @error('blood_type_id')<div class="text-danger">{{ $message }}</div>@enderror
            ...
            <input class="main-font form-control my-4 rounded-0" type="number" name="bags_number" id="bags_number"
                placeholder="{{ __('messages.bags_number') ?? 'Bags number' }}" value="{{ old('bags_number') }}" min="1" />
            @error('bags_number')<div class="text-danger">{{ $message }}</div>@enderror
            ...
            @error('hospital_name')<div class="text-danger">{{ $message }}</div>@enderror
            ...
            @error('patient_phone')<div class="text-danger">{{ $message }}</div>@enderror
            ...
            <div class="d-flex gap-2 mb-3">
                <input class="main-font form-control" type="text" id="latitude_display" readonly value="{{ old('latitude', $initLat) }}">
                <input class="main-font form-control" type="text" id="longitude_display" readonly value="{{ old('longitude', $initLng) }}">
            </div>
            ...
            <textarea class="main-font form-control my-4 rounded-0" name="notes" id="notes" rows="4" placeholder="{{ __('messages.notes') ?? 'Notes' }}">{{ old('notes') }}</textarea>
            @error('notes')<div class="text-danger">{{ $message }}</div>@enderror

            <button type="submit" class="main-font btn border rounded-0 d-block mx-auto">{{ __('messages.send') ?? 'Send' }}</button>
        </form>
    </section>
@endsection
