            ...
            <div class="col-md-6">
                <div class="card">
                    ...
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h3 class="card-title">{{ __('messages.posts_list') }}</h3>
            <a href="{{ route('posts.create') }}" class="btn btn-primary btn-sm ml-auto"><i
                        ...
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th>{{ __('messages.post_content') }}</th>
                                    <th>{{ __('messages.post_photo') }}</th>
                            ...
                            <tbody>
                                @foreach ($posts as $post)
                                    <tr id="tr_{{ $post->id }}">
                                            ...
                                            <td>
                                                @if ($post->photo)
                                                    <img src="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($post->photo) }}" alt="{{ $post->title }}" class="img-thumbnail" style="max-width: 100px;">
                                                @endif
                                            </td>
                                        <td class="d-flex">
                                            <a href="{{ route('posts.edit', $post->id) }}"
                                                class="btn btn-primary mx-1"><i class="far fa-edit"></i></a>
                                            <button type="button" data-id="tr_{{ $post->id }}"
